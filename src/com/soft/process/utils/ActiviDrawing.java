 ...
 * @author Joram Barrez
 */
public class ActiviDrawing {

  protected static final Logger LOGGER = Logger.getLogger(ActiviDrawing.class.getName());
   ...
   */
  public ActiviDrawing(int width, int height) {
   ...
   *          below minX on the horizontal scale will be cropped.
   */
  public ActiviDrawing(int width, int height, int minX, int minY) {
    this(width, height);
    this.minX = minX;
    this.minY = minY;
  }
  ...
  static Color colorHistory=Color.BLUE;
  //TODO
  public void drawSequenceflowHistory(int srcX, int srcY, int targetX, int targetY, boolean conditional) {
	    Line2D.Double line = new Line2D.Double(srcX, srcY, targetX, targetY);
	    Graphics2D   g = processDiagram.createGraphics();
	    g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
	    g.setPaint(colorHistory);
	    g.draw(line);
	    drawArrowHeadHistory(line);
	    if (conditional) {
	      drawConditionalSequenceFlowIndicatorHistory(line);
	    }
	  }

	  public void drawSequenceflowWithoutArrowHistory(int srcX, int srcY, int targetX, int targetY, boolean conditional) {
	    Line2D.Double line = new Line2D.Double(srcX, srcY, targetX, targetY);
	    Graphics2D   g = processDiagram.createGraphics();
	    g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
	    g.setPaint(colorHistory);
	  ...
	  public void drawArrowHeadHistory(Line2D.Double line) {
	    int doubleArrowWidth = 2 * ARROW_WIDTH;
	    Polygon arrowHead = new Polygon();
	    arrowHead.addPoint(0, 0);
	    arrowHead.addPoint(-ARROW_WIDTH, -doubleArrowWidth);
	  ...
	  public void drawConditionalSequenceFlowIndicatorHistory(Line2D.Double line) {
	    int horizontal = (int) (CONDITIONAL_INDICATOR_WIDTH * 0.7);
	    int halfOfHorizontal = horizontal / 2;
	    int halfOfVertical = CONDITIONAL_INDICATOR_WIDTH / 2;
  ...
  public void drawHighLightHistory(int x, int y, int width, int height) {
	    Paint originalPaint = g.getPaint();
	    Stroke originalStroke = g.getStroke();
	    g.setPaint(colorHistory);
	    g.setStroke(THICK_TASK_BORDER_STROKE);
